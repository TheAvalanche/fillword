...
FillwordComposer.prototype.addWords = function (words) {

    for (let i = 0; i < words.length; i++) {
        this.addWord(words[i].toUpperCase());
    }
...
FillwordComposer.prototype.decorate = function (table) {
    for (var i = 0; i < this.grid.length; i++) {
        var row = table.insertRow();
        for (var k = 0; k < this.grid[i].length; k++) {
            var cell = row.insertCell();
            cell.innerHTML = this.grid[i][k];
        }
    }
};
